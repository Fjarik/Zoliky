$(() => {
	...
	$("#Email").rules("add",
		{
			email: true,
			messages: {
				email: "Musíte zadat platný email"
			}
		});
	$("#PasswordRepeat").rules("add",
		{
			equalTo: "#Password",
			messages: {
	...
	$(".validation-wizard").steps({
		...
		onStepChanging: function(event, currentIndex, newIndex) {
			...
			return currentIndex > newIndex ||
				...
				(form.find(".body:eq(" + newIndex + ") label.error").remove(), form
					.find(".body:eq(" + newIndex + ") .error").removeClass("error")), form.validate().settings.ignore =
	...
	$("input[data-val='true']").on("keydown", blockSpace);
	$("button#btn-generate").click(gen);
