...
/*
  Future<MActionResult<Transaction>> transferAsync(ZolikPackage p) async {
    if (p == null || !p.isValid) {
    ...
    try {
      ...
      MActionResult<Transaction> ws = MActionResult.fromJson(_json);
      return Future<MActionResult<Transaction>>.value(ws);
    } catch (ex) {
  ...
  Future<MActionResult<List<Zolik>>> getUserZoliksAsync(int userId,
    ...
    try {
      ...
      ws.content = zoliks;
      return Future<MActionResult<List<Zolik>>>.value(ws);
    } catch (ex) {
